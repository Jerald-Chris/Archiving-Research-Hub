...
function add_HUMMS(){
  ...
  if(isset($_POST['register_submit'])){
    ...
    //condition if research title already exist
    $select_query = "Select *from `research_humms` where Research_Title = '$title'";
    $result = mysqli_query($con, $select_query);
    $rows_count = mysqli_num_rows($result);
    if($rows_count>0){
        echo "<script> alert('Research already exist')</script>";
        ...
        //insert research files
        $insert_query = "insert into `research_humms` (Research_Title, Author, Research_Image, Strand, Date_Added)
        values ('$title', '$author', '$image', '$strand', NOW())";
        $sql_execute = mysqli_query($con, $insert_query);
        echo"<script> alert('Successfully Added')</script>";
    }
        ...
        foreach($query_run as $row){
          ...
          ?>
            <tr>
                ...
                <td>
                      <button><a href="../index/delete.php?ID=<?php echo $row['ID']; ?>">Delete</a></button>
                </td>
            </tr>
          <?php
        }   
